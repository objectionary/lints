...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:eo="https://www.eolang.org" version="2.0" id="object-has-data">
  ...
  <xsl:template match="/">
    <defects>
      <xsl:for-each select="//o[eo:has-data(.) and not(parent::o[@base='Φ.bytes'])]">
        <defect>
          ...
          <xsl:text>Only object that have parent object with @base equal to "Φ.bytes" may contain data, while</xsl:text>
          <xsl:choose>
