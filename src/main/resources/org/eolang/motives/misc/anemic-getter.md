...
does not. A name written once as `f` stays `f` however the formation is
nested later, while `^.^` points at whatever happens to be two hops up
if a block using it is re-indented or wrapped in another formation.
